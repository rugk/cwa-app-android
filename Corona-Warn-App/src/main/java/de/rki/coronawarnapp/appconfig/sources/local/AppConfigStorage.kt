...
import de.rki.coronawarnapp.util.serialization.writeToFile
...
class AppConfigStorage @Inject constructor(
    ...
    suspend fun getStoredConfig(): InternalConfigData? = mutex.withLock {
        ...
        return@withLock try {
            objectMapper.readValue<InternalConfigData>(configFile).also {
    ...
    suspend fun setStoredConfig(value: InternalConfigData?): Unit = mutex.withLock {
        ...
        try {
            objectMapper.writeToFile(configFile, value)
        } catch (e: Exception) {
